...
API_TOKEN = '7940195181:AAHUrp4N2hfI4lPNoybmo5ItBOi0puz83qU'
...
async def get_username_by_telegram_id(telegram_id):
    async with Bot(token=API_TOKEN) as bot:
        try:
            chat = await bot.get_chat_member(telegram_id, telegram_id)
...
async def insertion_send_telegram_notification(master_id, user_telegram_id, booking_data):
    async with Bot(token=API_TOKEN) as bot:
        ...
        try:
            await bot.send_message(chat_id=master_telegram_id, text=message)
        except Exception as e:
            pass

async def delete_send_telegram_notification(master_id, user_id, booking_data):
    async with Bot(token=API_TOKEN) as bot:
        master_telegram_id = await get_telegram_id_by_master_id(master_id)
